            ...
            if (livePreview) {
                ...
                if (song.columnCount) {
                    ...
                    if (columnCountSelect) {
                        ...
                        livePreview.style.columnCount = song.columnCount;
...
[C2]Great are [D]You, [G]Lord`;
        ...
        if (layoutMatch && layoutMatch[1]) {
            ...
            if (layoutValue === 1 || layoutValue === 2) {
                ...
                if (columnCountSelect && livePreview) {
                    ...
                    livePreview.style.columnCount = layoutValue.toString();
    ...
    function autoOptimizeLayout() {
        ...
        livePreview.style.columnCount = '1';
    ...
    function applyLayoutSettings() {
        ...
        // Apply column settings (with webkit prefixes for Safari)
        livePreview.style.webkitColumnCount = columns.toString();
        livePreview.style.MozColumnCount = columns.toString();
        livePreview.style.columnCount = columns.toString();
        livePreview.style.columns = columns.toString();
        livePreview.style.webkitColumnFill = 'auto';
        livePreview.style.MozColumnFill = 'auto';
        livePreview.style.columnFill = 'auto';
        livePreview.style.height = height + 'px';
        livePreview.style.maxHeight = height + 'px';
        livePreview.style.overflow = 'hidden';

        if (columns > 1) {
            livePreview.style.webkitColumnGap = '40px';
            livePreview.style.MozColumnGap = '40px';
            livePreview.style.columnGap = '40px';
        } else {
            livePreview.style.webkitColumnGap = '0px';
            livePreview.style.MozColumnGap = '0px';
    ...
    */
    ...
    window.addEventListener('songLoaded', async (event) => {
        ...
        if (loadedBaseline) {
            ...
            // Restore saved transpose steps (or 0 if not saved)
            currentTransposeSteps = detail.transposeSteps || 0;
            console.log('Restored transposeSteps:', currentTransposeSteps);
            ...
            // If song was saved with transpose, apply it now
            if (currentTransposeSteps !== 0) {
                console.log('🎼 Applying saved transpose:', currentTransposeSteps, 'steps');
                // Transpose the visual format
                visualFormat = transposeVisualFormat(visualFormat, currentTransposeSteps);
                // Also transpose the songbook/normalized format
                const transposedNormalized = transposeChart(normalizedChart, currentTransposeSteps);
                songbookOutput.value = transposedNormalized;
                setDirectionalLayout(songbookOutput, transposedNormalized);
            }
            ...
            if (livePreview) {
                ...
                if (detail.columnCount) {
                    ...
                    if (columnCountSelect) {
                        ...
                        livePreview.style.columnCount = detail.columnCount;
