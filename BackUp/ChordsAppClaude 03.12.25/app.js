...
[C2]Great are [D]You, [G]Lord`;
    ...
    function autoInsertArrangementLine(content) {
        ...
        for (let line of lines) {
            ...
            if (match) {
                ...
                // Map to short form
                let shortForm = sectionType;
                if (sectionType === 'VERSE') shortForm = 'V';
                else if (sectionType === 'CHORUS') shortForm = 'C';
                else if (sectionType === 'BRIDGE') shortForm = 'B';
                else if (sectionType === 'INTRO') shortForm = 'I';
                else if (sectionType === 'OUTRO') shortForm = 'O';
                else if (sectionType === 'PRE-CHORUS') shortForm = 'PC';
                else if (sectionType === 'TAG') shortForm = 'T';
                else if (sectionType === 'CODA') shortForm = 'CD';

                const badge = sectionNum ? `(${shortForm}${sectionNum})` : `(${shortForm})`;
                sections.push(badge);
            }
        }

        if (sections.length === 0) {
            return content; // No sections found, return original
        }

        // Build the arrangement line
        const arrangementLine = sections.join(' ');
    ...
    async function savePrintPreviewPreferences() {
        ...
        try {
            // Get current column layout setting
            const activeColumnBtn = document.querySelector('.column-btn.active');
            const columnCount = activeColumnBtn ? parseInt(activeColumnBtn.dataset.columns) : 1;
            ...
            if (preferences) {
                ...
                if (preferences.columnLayout) {
                    ...
                    columnButtons.forEach(btn => {
                        ...
                        if (btnColumns === preferences.columnLayout) {
                            btn.classList.add('active');
                            if (livePreview) {
                                livePreview.style.columns = preferences.columnLayout;
                            }
                        } else {
